...
std::string tokenizeSpecialCharacters(int* i, std::string* file){
	...
	for(int i2=0;i2<specialCharacters.size();i2++){
		if(file->size() >  specialCharacters[i2].size()-1 + *i){//check to make sure the file is long enough to get the substr
			if(file->substr(*i, specialCharacters[i2].size()) == specialCharacters[i2]){
